...
$$
C_i = \frac{1}{n} \sum_{j=1}^n \frac{V_{\text{input},ij} \cdot V_{\text{keyphrases},ij}}{\left\|V_{\text{input},ij}\right\| \left\|V_{\text{keyphrases},ij}\right\|}
$$

- \( C_i \): Coherence score for the \(i\)-th document  
- \( n \): Number of dimensions in the embedding space  
- \( V_{\text{input},ij} \): The \(j\)-th dimension of the normalized embedding vector for the input text of the \(i\)-th document  
- \( V_{\text{keyphrases},ij} \): The \(j\)-th dimension of the normalized embedding vector for the keyphrases of the \(i\)-th document  
- \( \cdot \): Dot product  
- \( \|v\| \): Euclidean norm (or magnitude) of vector \(v\)
